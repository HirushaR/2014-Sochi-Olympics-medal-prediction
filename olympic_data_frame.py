...
olympic_medal_count_df = { 'Country_name' : pd.Series(contries), 'Gold': pd.Series(Gold), 'Silver': pd.Series(Silver), 'Bronze': pd.Series(Bronze)}

df = pd.DataFrame(olympic_medal_count_df)
print(df)
